...
func (cm *ClusterManager) GetClusterSnapshot(clusterID string) (
	...
	if !ok {
		return nil, nil, nil, nil, nil, nil, nil, nil, nil, fmt.Errorf("cluster %s not found", clusterID)
	}
	pods, nodes, services, deployments, replicaSets, statefulSets, daemonSets, ingresses, incidents := im.GetSnapshot()
	return pods, nodes, services, deployments, replicaSets, statefulSets, daemonSets, ingresses, incidents, nil
}
