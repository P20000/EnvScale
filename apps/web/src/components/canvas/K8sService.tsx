...
export const K8sServiceNode = memo(({ id, data, selected }: NodeProps & { data: K8sServiceData }) => {
  ...
  return (
    <div
      className={`relative flex items-center gap-3 rounded-full bg-[#141417] px-4 py-2 border text-left shadow-lg transition-all ${
        selected
    ...
    >
      ...
      <div className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20 shrink-0">
      ...
      <div className="flex flex-col min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-mono text-blue-400 font-bold">{data.port}</span>
        </div>
        <span className="text-[10px] text-neutral-400 font-mono">{data.type}</span>
      </div>
